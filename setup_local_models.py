...
Downloads (~3.8 GB total), no fallback models -- config.py has no fallback
path, so a weaker backup model would only cost image size with no upside:
    generalist : Qwen2.5-3B-Instruct        (3B, Q4_K_M, ~1.9 GB)
